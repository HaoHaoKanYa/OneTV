...
import androidx.media3.exoplayer.rtsp.RtspMediaSource // 导入RtspMediaSource类，用于RTSP媒体源
...
class Media3VideoPlayer(
    ...
    private fun getMediaSource(uri: Uri, contentType: Int? = null): MediaSource? {
        ...
        if (uri.toString().startsWith("rtp://")) { // 如果是rtp协议
            return RtspMediaSource.Factory().createMediaSource(mediaItem) // 返回Rtsp媒体源
        }

        return when (val type = contentType ?: Util.inferContentType(uri)) { // 根据类型选择媒体源
            C.CONTENT_TYPE_HLS -> {
                HlsMediaSource.Factory(dataSourceFactory).createMediaSource(mediaItem) // HLS媒体源
            }

            C.CONTENT_TYPE_RTSP -> {
                RtspMediaSource.Factory().createMediaSource(mediaItem) // RTSP媒体源
            }
                ...
                androidx.media3.common.PlaybackException.ERROR_CODE_PARSING_CONTAINER_UNSUPPORTED -> {
                    videoPlayer.currentMediaItem?.localConfiguration?.uri?.let {
                        if (contentTypeAttempts[C.CONTENT_TYPE_HLS] != true) {
                            ...
                            prepare(it, C.CONTENT_TYPE_RTSP) // 尝试RTSP解析
    ...
    private val metadataListener = object : AnalyticsListener {
        override fun onVideoInputFormatChanged(
            ...
            metadata = metadata.copy(
                videoMimeType = format.sampleMimeType ?: "", // 获取视频格式
                videoWidth = format.width, // 获取视频宽度
                videoHeight = format.height, // 获取视频高度
                videoColor = format.colorInfo?.toLogString() ?: "", // 获取视频颜色信息
                // TODO 帧率、比特率目前是从tag中获取，有的返回空，后续需要实时计算
                videoFrameRate = format.frameRate, // 获取帧率
                videoBitrate = format.bitrate, // 获取比特率
            )
        ...
        override fun onVideoDecoderInitialized(
            ...
            triggerMetadata(metadata) // 获取视频解码器信息
        }
        ...
        override fun onAudioInputFormatChanged(
            ...
            metadata = metadata.copy(
                audioMimeType = format.sampleMimeType ?: "", // 获取音频格式
                audioChannels = format.channelCount, // 获取音频频道数
                audioSampleRate = format.sampleRate, // 获取音频采样率
            )
            triggerMetadata(metadata) // 触发元数据回调
        }
        ...
        override fun onAudioDecoderInitialized(
            ...
            triggerMetadata(metadata) // 获取音频解码器信息
        }
    }
